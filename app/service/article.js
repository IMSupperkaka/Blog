...
class ArticleService extends Service {
    ...
    // }
    async list({ offset, limit, page, pageSize }, { type }) {
        let condition = '';
        if (!this.dataEmpty(type)) {
            condition = ` where b.type = '${type}' `;
        }
        let getSqlData = await this.pageData(
            `SELECT a.id,a.tags,a.title,b.type,b.name categoryName,a.descripe,a.content,a.hit_nums hitNums,a.create_time publishTime,a.name authorName,c.nums commentNums FROM category as b 
            right join (
            select ia.*,u.name from users u  
            right join (select id,title,tags,descripe,content,hit_nums,create_time,c_id,author_id,sort from article where status = 1) as ia on ia.author_id = u.id
            ) as a on a.c_id = b.id 
